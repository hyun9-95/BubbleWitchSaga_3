...
public class AudioClipSupport : MonoBehaviour
{
	...
	{
		if (audioClipPathArray == null)
			return;

		int index = Random.Range(0, audioClipPathArray.Length);

		PlayPathFromArray(index);
	}
    ...
    public void PlayPathFromArray(int index)
	{
		...
		SoundManager.Instance.PlaySound(soundType, path, volumeMultiplier: volumeMultiplier).Forget();
	}
    ...
    private void AutoPlay()
	{
        ...
        SoundManager.Instance.PlaySound(soundType, autoPlayPath, volumeMultiplier: volumeMultiplier).Forget();
	}
	#endregion
}
