...
public class SoundManager : BaseMonoManager<SoundManager>
{
    ...
    private string titleBgmPath;
    private bool isFading = false;
    private Func<float> OnGetMasterVolume;
    private Func<SoundType, float> OnGetVolume;

    public void SetOnGetVolume(Func<float> onGetMasterVolume, Func<SoundType, float> onGetVolume)
    {
        OnGetMasterVolume = onGetMasterVolume;
    ...
    public async UniTask PlaySound(SoundType soundType, string path, Vector3 pos = default, float pitch = 1, float volumeMultiplier = 1)
    {
        ...
        if (audioSource.clip != null)
        {
            audioSource.volume = GetVolume(soundType) * volumeMultiplier;
            audioSource.pitch = pitch;
            audioSource.Play();
        }

        audioSourcePool.AutoReturnToPool(audioSource).Forget();
    }
    ...
    public void Clear()
    {
        for (int i = 0; i < audioSourcePools.Length; i++)
            audioSourcePools[i].Clear();
    }

    private float GetVolume(SoundType soundType)
    {
        var masterVolume = OnGetMasterVolume != null ?
            OnGetMasterVolume() : 1.0f;

        float volume = OnGetVolume != null ?
            OnGetVolume(soundType) : 1.0f;
